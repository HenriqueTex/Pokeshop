...
export function CollectionPage() {
  ...
  const bannerUrl = collection?.bannerUrl ? mediaUrl(collection.bannerUrl) : undefined
  ...
  return (
    <main className="shop-page">
      ...
      {collection && <>
        <section className={`collection-hero${bannerUrl ? ' collection-hero--with-image' : ''}`} style={bannerUrl ? { backgroundImage: `linear-gradient(90deg, rgb(7 7 6 / 92%) 0%, rgb(7 7 6 / 66%) 39%, rgb(7 7 6 / 12%) 79%), linear-gradient(0deg, #0a0a0a 0%, transparent 42%), url(${bannerUrl})` } : undefined}><p className="eyebrow">Coleção selecionada</p><h1>{collection.name}</h1>{collection.description && <p>{collection.description}</p>}<Link className="gold-link" to={`/catalogo?collection=${collection.slug}`}>Filtrar no catálogo →</Link></section>
