...
export function saveAdminProduct(input: AdminProductInput, id?: number) {
  return adminRequest<{ data: Product }>(
    `/api/v1/admin/products${id ? `/${id}` : ""}`,
    {
      method: id ? "PUT" : "POST",
      body: JSON.stringify({
        ...input,
...
export function deleteAdminProduct(id: number) {
  return adminRequest<void>(`/api/v1/admin/products/${id}`, {
    method: "DELETE",
  });
}
...
export function saveAdminCollection(input: AdminCollectionInput, id?: number) {
  return adminRequest<{ data: Collection }>(
    `/api/v1/admin/collections${id ? `/${id}` : ""}`,
    {
      method: id ? "PUT" : "POST",
      body: JSON.stringify({
        ...input,
...
export function deleteAdminCollection(id: number) {
  return adminRequest<void>(`/api/v1/admin/collections/${id}`, {
    method: "DELETE",
  });
}
...
export function saveAdminBanner(input: AdminBannerInput, id?: number) {
  return adminRequest<{ data: PromotionalBanner }>(
    `/api/v1/admin/banners${id ? `/${id}` : ""}`,
    {
      method: id ? "PUT" : "POST",
...
export function deleteAdminBanner(id: number) {
  return adminRequest<void>(`/api/v1/admin/banners/${id}`, {
    method: "DELETE",
  });
}
...
export async function uploadAdminImage(file: File) {
  ...
  const response = await fetch(`${apiUrl}/api/v1/admin/uploads`, {
    method: "POST",
    credentials: "include",
    body: formData,
  });
